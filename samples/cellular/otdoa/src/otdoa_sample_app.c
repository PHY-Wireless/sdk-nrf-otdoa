...
static void pos_est_timer_restart();
...
static void display_result_details(const otdoa_api_result_details_t* p_details)
{
    ...
    for (unsigned u=0; u < p_details->num_measured_cells; u++)
    {
        if (p_details->toa_detect_count[u] > 0)
        {
            LOG_INF("    ecgi: %u  detects: %u", p_details->ecgi_list[u], p_details->toa_detect_count[u]);
        }
    }
}
    ...
    switch (p_event_data->event)
    {
        ...
        case OTDOA_EVENT_UBSA_DL_COMPL:
        {
            ...
            if (dl_in_session)
            {
                dl_in_session = 0;
                set_blink_prs();
                int err = otdoa_api_ubsa_available(p_event_data->dl_compl.status, UBSA_FILE_PATH);
                if (err != OTDOA_API_SUCCESS)
                {
...
// callback for periodic position estimates time
static void pos_est_timer_cb(struct k_timer *timer) {
...
static void pos_est_timer_stop() {
