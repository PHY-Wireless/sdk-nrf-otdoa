 ...
 * otdoa_thread_http.c
 *
 * This file contains nordic utilities for the HTTP module
 *
 */
...
void http_modem_info_init()
{
    if (!bModemInfoInit) {
        modem_info_init();
        // modem_info_params_init(&MPI);
        bModemInfoInit = true;
    }
}
...
int is_ip_valid(tOTDOA_HTTP_MEMBERS *pG) {
    ...
    modem_info_string_get(MODEM_INFO_IP_ADDRESS, pG->szModemAddress, sizeof(pG->szModemAddress));
...
int tls_setup(int fd, const char* host) {
    ...
    if (!server) server = otdoa_http_get_download_url();
...
int http_bind(tOTDOA_HTTP_MEMBERS *pG, const char* pURL) {
    ...
    if (pURL == NULL) pURL = otdoa_http_get_download_url();
    ...
    for (nRetry = 0; nRetry < MAX_BIND_RETRIES; nRetry++) {
        ...
        if ((rc = getaddrinfo(pURL, NULL, &hints, &pG->res)) == 0)
            break;
        //if (nRetry % 30 == 0)
        //    OTDOA_LOG_ERR("getaddrinfo() retry %d failed, err: %s", nRetry,
        //            rc == EAI_SYSTEM ? strerror(errno) : gai_strerror(rc));
        // feed the watchdog
        //otdoa_wdt_feed(WDT_CHANNEL_HTTP);
        k_sleep(K_SECONDS(1));
    }
    ...
    // #ifdef VERBOSE
    {
        ...
        while (info) {
            if (!inet_ntop(AF_INET, &((struct sockaddr_in *)pG->res->ai_addr)->sin_addr, pG->szServerAddress, sizeof pG->szServerAddress)) {
                OTDOA_LOG_ERR("Failed to convert address to text form: %d %s", errno, strerror(errno));
                return -1;
            } else {
 ...
 * @return 0 for success, -1 for socket open failure, -2 for tls_setup failure, -3 for connect failure
 */
int http_connect(tOTDOA_HTTP_MEMBERS *pG, const char* tls_host) {
...
#ifdef TIMING
    int64_t unix_time_ms_start = k_uptime_get();
#endif
    ...
    if (nErr) {
        ...
        OTDOA_LOG_ERR("connect failed: fdSocket = %d, ai_addr = %p", pG->fdSocket, (void*)pG->res->ai_addr);
        http_disconnect(pG);
        return -3;
    }
#ifdef TIMING
    int64_t unix_time_ms_finish = k_uptime_get();
    unsigned elapsed = unix_time_ms_finish - unix_time_ms_start;
    OTDOA_LOG_DBG("INFO execution time %dms", elapsed);
#endif
    return nErr;
}
...
bool SetSocketBlocking(int fd, bool blocking) {
    ...
    //bool rv = (fcntl(fd, F_SETFL, flags) == 0) ? true : false;
...
// wrappers
ssize_t http_recv(int socket, void *buffer, size_t length, int flags) { return recv(socket, buffer, length, flags); }
ssize_t http_send(int socket, const void *buffer, size_t length, int flags) {
    return send(socket, buffer, length, flags);
}

int http_errno(void) { return errno; }
