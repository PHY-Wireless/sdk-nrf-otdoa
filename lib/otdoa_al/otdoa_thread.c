...
int otdoa_start() {
    ...
    const unsigned int MAX_MSG_SIZE = MAX(OTDOA_MAX_MESSAGE_SIZE, sizeof(tOTDOA_HTTP_WORK));
    ...
    k_thread_create(&rs_thread_data, rs_thread_stack, K_THREAD_STACK_SIZEOF(rs_thread_stack), rs_entry_point, NULL, NULL, NULL, CONFIG_OTDOA_RS_THREAD_PRIORITY, K_FP_REGS, K_NO_WAIT);
...
void otdoa_message_free(void* msg) {
...
int otdoa_queue_http_message(const void* msg, const size_t length) {
    ...
    tOTDOA_HTTP_WORK* work = otdoa_message_alloc(sizeof (struct k_work) + length);
    if (!work) {
    ...
    if (rc != 1) {
    	// 0 and 2 are success values, but imply the work item has be double-allocated
...
bool otdoa_message_check_pending_stop() {
    ...
    if (pending) k_work_cancel(&http_stop_work.work);
    return pending;
}
