 ...
 *------------------------------------------------------------------+
 *
 * otdoa_nordic_at_h1.c - AT command/response
...
#define REG_STATUS_NONE         0  // Not registered. UE is not currently searching for an operator to register to.
#define REG_STATUS_REGISTERED   1  // Registered, home network
#define REG_STATUS_NOT_REG      2  // Not registered, but UE is currently trying to attach or searching an operator torester to
#define REG_STATUS_DENIED       3  // Registration denied
#define REG_STATUS_UNKNOWN      4  // Unknown (for example, out of E-UTRAN coverage)
#define REG_STATUS_ROAMING      5  // Registered, roaming
#define REG_STATUS_UICC_FAIL    90 // Not registered due to UICC failure
        ...
        uint32_t* pu32_ecgi, uint32_t* pu32_dlearfcn)
{
    ...
    for (token = 1+otdoa_nordic_at_strtok_r(token, s, &pstrCopy);
        token != NULL;
        token = otdoa_nordic_at_strtok_r(pstrCopy, s, &pstrCopy))

#else
    const char s[] = ",:";
    char *token = strstr(strCopy, " ");   //  Skip the AT string name
    for (token = 1+strtok(token, s); token != NULL; token = strtok(NULL, s))
#endif
    {
        // OTDOA_LOG_DBG("  TOKEN %d : %s\n", n_token, token);
        switch (n_token)
        {
        ...
        if (i_ret)    //  Don't continue after an error
        {
...
error_exit:
    if (0 == i_ret) // if no errors in parsing
    {
    ...
    {
        if (psz_resp) OTDOA_LOG_ERR("AT%%XMONITOR response %s", psz_resp);
        OTDOA_LOG_ERR("Failed to parse AT%%XMONITOR response.  returning %d\n", i_ret);
    }
    else
    {
        OTDOA_LOG_DBG("otdoa_nordic_at_parse_xmonitor_response: ECGI=%"PRIu32" (0x%08X), DLEARFCN=%"PRIu32" returning %d\n",
            u32_egci, (unsigned)u32_egci, u32_dlearfcn, i_ret);
    }
...
int otdoa_nordic_at_get_ecgi_and_dlearfcn( uint32_t* pu32_ecgi, uint32_t* pu32_dlearfcn)
{
    ...
    if (i_ret)
    {
        OTDOA_LOG_ERR("otdoa_nordic_at_get_ecgi_and_dlearfcn: ERROR (%d) Failed to get MODEM Status\n", i_ret);
        i_ret = OTDOA_EVENT_FAIL_BAD_MODEM_RESP;
    }
    else
    {
...
// The AT command query we will use.  Note that "=1" ensures we get the IMEI value (see AT command reference)
...
int otdoa_nordic_at_get_imei_from_modem()
{
    ...
    if (!p_imei_token || strnlen(p_imei_token,CGSN_RESPONSE_LENGTH) < (NRF_IMEI_LEN+IMEI_RESP_LEN))
    {
        goto error_return;
    }
    ...
    if (!p_imei_end) goto error_return;         // no closing quote

    size_t len = p_imei_end - p_imei_token;
    if (len < NRF_IMEI_LEN) goto error_return;  // IMEI too short
    if (len > NRF_IMEI_LEN) len = NRF_IMEI_LEN;
