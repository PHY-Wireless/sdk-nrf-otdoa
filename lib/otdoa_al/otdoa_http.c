...
void otdoa_http_defaults(tOTDOA_HTTP_MEMBERS *pG) {
    pG->uEcgi = 10242563;
    pG->uDlearfcn = 5230; // Shold probably use PXL_DEFAULT_UBSA_DLEARFCN but it's not in scope
    pG->uRadius = 3000;
    pG->uNumCells = 50;
...
int otdoa_http_handle_message(tOTDOA_HTTP_MESSAGE *pMsg) {
    ...
    case OTDOA_HTTP_MSG_REBIND_SOCKET:
        ...
        /* rebind the socket in case the configured upload URL has changed. has to be a message because
           config file parsing is slow and I can't just do this at the end of GET_CONFIG_FILE above */
...
int otdoa_http_send_message(tOTDOA_HTTP_MESSAGE*pMsg) {
    // set msb of message type to indicate it is for control FSM
