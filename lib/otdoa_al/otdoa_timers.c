...
{
    for (unsigned ui=0; ui < OTDOA_MAX_TIMERS; ui++)
    {
...
{
    if (uTimerNo >= OTDOA_MAX_TIMERS)
        return -1;

    // timer is started as one-shot, so period=K_NO_WAIT
    k_timer_start(otdoa_timers + uTimerNo, K_MSEC(uDurationMS), K_NO_WAIT);
    return 0;
}
...
int otdoa_timer_stop(unsigned uTimerNo)
{
    ...
    k_timer_stop(otdoa_timers + uTimerNo);
    return 0;
}
...
int otdoa_timer_active(unsigned uTimerNo)
{
    if (uTimerNo >= OTDOA_MAX_TIMERS)
        return 0;
    if (k_timer_remaining_ticks(otdoa_timers + uTimerNo) > 0)
        return 1;
    return 0;
}
