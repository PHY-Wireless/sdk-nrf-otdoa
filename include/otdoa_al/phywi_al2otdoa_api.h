 ...
 */
void otdoa_ctrl_handle_message(void* pvMsg);    // Need to rename
extern const unsigned OTDOA_MAX_MESSAGE_SIZE;
 ...
 *          0              valid        Data encrypted by server.  Store IV from server along with the data
 *          0              NULL         Store unencrypted data.  IV set to 0s to indicate data is unencrypted on read
 *          1               X           Data NOT encrypted by server.  Encrypt it here.  IV generated locally
 */
int otdoa_ubsa_proc_start_h1(const char* psz_path, 
                             int enable_ue_encr,        // encrypt uBSA locally on UE (i.e. it is not ecrypted by server)
                             int is_compressed, 
                             uint32_t u32_comp_window,  
 ...
 */
int otdoa_ubsa_finish_data();

void otdoa_crypto_abort();
void otdoa_ubsa_proc_close_file();
void otdoa_ubsa_proc_remove_file(const char* pszFile);

const char *OTDOA_pxlGetBSAPath();
/**
